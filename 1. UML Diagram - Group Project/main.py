...
class Contact:
    def __init__(self, name: str, address: str, phone: str, email: str, birthday: str | date):
        self.name = name
...
class FolderOrganizer:
    ...
    def organize_folder(self, local_path: str):
        ...
        if not self.folder_path.exists() or not self.folder_path.is_dir():
            ...
            self.console.print(f'[green]Файли в папці "{self.folder_path.name}" відсортовані.[/green]')
...
class PersonalAssistant:
    def __init__(self, validation_class: DataEntryValidation, folder_organizer_class: FolderOrganizer):
    ...
    def add_contact_from_console(self) -> None:
        ...
        while True:
            try:
                ...
                self.console.print(
                    "[bold red]Помилка:[/bold red] Некоректний формат номера телефону. Використовуйте наступний формат: +380501234567, 050-123-45-67, 0501234567 або (050)123-45-67.")
        while True:
            try:
                ...
                self.console.print("[bold red]Помилка:[/bold red] Некоректна електронна пошта. Спробуйте ще раз.")
        # Додатково: питаємо користувача про день народження і дозволяємо різні формати
        while True:
            try:
                birthday = parser.parse(input("Дата народження (день-місяць-рік): ")).date()
                break  # Якщо парсинг відбувся успішно, виходимо з циклу
            except ValueError:
                self.console.print("[bold red]Помилка:[/bold red] Некоректний формат дати. Спробуйте ще раз.")
        self.add_contact(name, address, phone, email, birthday)

    def add_contact(self, name: str, address: str, phone: str, email: str, birthday: str | date) -> None:
        ...
        if existing_contact:
            self.console.print(f"[bold red]Помилка:[/bold red] Контакт з такими номерами телефонів вже існує.")
            return
        # Додавання нового контакту до книги контактів
        new_contact = Contact(name, address, phone, email, birthday)
        self.contacts.append(new_contact)
        self.console.print(f"[green]Контакт {name} успішно доданий до книги контактів.[/green]")
    ...
    def upcoming_birthdays(self, days: int) -> None:
        ...
        if not upcoming_birthdays:
            self.console.print(f'[yellow]У {days} днів немає найближчих днів народження.[/yellow]')
        else:
    ...
    def get_next_birthday(self, contact: Contact) -> date:
        """
    ...
    def search_contacts(self, query: str = None) -> Optional[Contact]:
        ...
        if matching_contacts:
            ...
            self.console.print(f"[red]Немає результатів пошуку для запиту: {query}[/red]")
            return None
    ...
    def edit_contact(self, contact: Contact) -> None:
        ...
        if new_phone:
            if self.validation.is_valid_phone(new_phone):
                ...
                self.console.print("[bold red]Помилка:[/bold red] Некоректний номер телефону.")
        ...
        if new_email:
            if self.validation.is_valid_email(new_email):
                ...
                self.console.print("[bold red]Помилка:[/bold red] Некоректна електронна пошта.")
        ...
        if new_birthday:
            try:
                ...
                self.console.print("[bold red]Помилка:[/bold red] Некоректний формат дати. Залишено попередню дату.")
        self.dump()
        self.console.print(f"[green]Контакт {contact.name} успішно відредаговано.[/green]")
    ...
    def delete_contact(self, contact: Contact = None) -> None:
        ...
        if contact in self.contacts:
            ...
            self.console.print(f"[green]Контакт {contact_name} успішно видалено.[/green]")
        else:
            self.console.print("[red]Помилка: Контакт не знайдено або не вибрано для видалення.[/red]")
    ...
    def list_notes(self) -> None:
        ...
        self.console.print(table, justify="center")
        self.console.print(f"[green]Кількість існуючих нотаток: {len(self.notes)}[/green]")
    ...
    def search_notes(self, text_query: str = None, tag_query: str = None) -> None:
        ...
        if matching_notes:
            ...
            if text_query is None:
                self.console.print(f"[red]Немає результатів пошуку за тегом: '{tag_query}'[/red]")
            if tag_query is None:
                self.console.print(f"[red]Немає результатів пошуку за текстом: '{text_query}'[/red]")
    ...
    def edit_note(self, note_index: int) -> None:
        ...
        if 0 <= note_index < len(self.notes):
            ...
            self.console.print(f"[green]Нотатка {note_index} успішно відредагована.[/green]")
        else:
    ...
    def delete_note(self) -> None:
        ...
        if matching_notes:
            ...
            if 0 < note_index <= len(matching_notes):
                ...
                self.console.print(f"[bold green]Нотатка успішно видалена:[/bold green] {deleted_note.text}")
            elif note_index == 0:
                self.console.print("[cyan]Видалення скасовано користувачем.[/cyan]")
            else:
                self.console.print("[red]Введено невірний номер нотатки. Видалення скасовано.[/red]")
        else:
            self.console.print(f"[red]Немає результатів пошуку для запиту: {query}[/red]")
    ...
    def analyze_user_input(self, user_input: str) -> None:
        ...
        if "додати контакт" in normalized_input:
            ...
            self.console.print("[red]Не можу розпізнати вашу команду. Пропоную Вам список доступних команд.[/red]")
